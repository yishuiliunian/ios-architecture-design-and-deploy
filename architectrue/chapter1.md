...
所以我们回过来头来看，架构是我们通过“结构”和“流程”来对抗复杂性的过程，其作用对象是工程本身和工程所处的组织及业务环境，终极目标是构建一个”耗散结构“来抵抗系统熵增导致的复杂性。而软件架构即是软件最顶层的结构设计。在客户端这个领域，我们所说的最顶层结构核心是组件结构和模块结构。

组件结构是物理单元，模块是逻辑单元。是从不同视角对于整个系统进行拆分。
...
可想而知，如果一个系统中充斥的是不可被认知的复杂性，势必会造成工程灾难，就相当于得了恶性肿瘤。而架构活动导致的复杂性，其实是”良性“的。看起来长了个瘤子，但是危害可能不大。但这依然是需要整个工程付出的成本。这个比喻，单纯是从成本方面进行考虑，从全局看可能并不是很恰当。

同时，有些做的比较的会落地很多的流程或者工具（服务），这可能会延长了原先的研发链路。例如原先提交个代码，只需要 `git commit` 就完成了，代码就算入库了。现在搞了一些流程，引入了静态检查、CodeReview巴拉巴拉，让代码入库的链路抻长。这些会让研发同学付出更多的时间和精力。这些也是成本。

经常被问的一个问题是：我们需不需要付出这些成本来博取对应的架构收益，做XXX事情的 ROI 收益如何？尤其是有些情况下会要求进行量化比较。进行量化（即度量）的确能够帮助我们更好的认知系统，但是度量本身也需要付出成本。而且相对于架构的成本而言，我更倾向于认为，这个问题场景下的”度量成本“是个恶性的，是个纯粹的成本。因为对于架构活动的成本评估，建议使用定性的方式，追求期有效性。同时，结合团队和业务的发展阶段来看，投入量。

一般我会预留30%的人力，来做“广义上”架构的事情。当然这个是个理想数值，在业务发展早期，可能会牺牲质量和工程的结构性来换取效率。需要投入的人力也少，但是只要业务度过了早期茹毛饮血的原始阶段，就需要立刻投入人力来做“架构”的事情了。要想长期发展，这部分的成本是必须投入的。
